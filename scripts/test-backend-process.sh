...
if [ "$(uname -s)" != Darwin ]; then
  printf '%s\n' 'Skipping backend process boundary tests outside macOS.'
  exit 0
fi
...
xcrun swiftc -swift-version 6 -parse-as-library \
  ...
  "$repo_root/scripts/test-support/BackendProcessTests.swift" \
  "${objects[@]}" -lsqlite3 -framework Security -framework LocalAuthentication \
  -o "$cache_root/BackendProcessTests"
"$cache_root/BackendProcessTests"
