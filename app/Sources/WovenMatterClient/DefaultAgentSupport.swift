...
public enum DefaultAgentSupport {
    ...
    public static var settings: DefaultAgentSettingsScope {
        ...
        set { if let data = try? JSONEncoder().encode(newValue) { UserDefaults.standard.set(data, forKey: settingsKey); changed() } }
    }
    ...
    public static func snapshot(workspace: String) throws -> DefaultAgentPayload {
        ...
        for id in ["openai", "openrouter", "opencode-go", "exa"] {
            if let key = try key(id, scope: keyScope) ?? (keyScope == "global" ? nil : key(id, scope: "global")), !key.isEmpty {
    ...
    public static func saveOAuth(_ credential: DefaultAgentCredential, provider: String, scope: String, notify: Bool = true, keychain: KeychainAccess = .init()) throws {
    ...
    @discardableResult
    public static func saveRenewedOAuth(_ credential: DefaultAgentCredential, replacing original: DefaultAgentCredential,
                                       provider: String, scope: String, keychain: KeychainAccess = .init()) throws -> Bool {
    ...
    public static func migrateLocalOAuth() throws {
        let path = FileManager.default.homeDirectoryForCurrentUser.appending(path: ".wovenmatter/default-agent/oauth.json")
        guard FileManager.default.fileExists(atPath: path.path) else { return }
        let values = try JSONDecoder().decode([String: DefaultAgentCredential].self, from: Data(contentsOf: path))
        for id in ["openai-codex", "xai"] {
            if let c = values[id], !c.borrowed, try oauth(id) == nil { try saveOAuth(c, provider: id, scope: "global", notify: false) }
        }
        // Delete only after every Keychain write has succeeded.
        try FileManager.default.removeItem(at: path)
    }

    public static func key(_ provider: String, scope: String = "global", keychain: KeychainAccess = .init()) throws -> String? {
        credentialLock.lock(); defer { credentialLock.unlock() }
