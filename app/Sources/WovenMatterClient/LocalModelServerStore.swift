...
public enum LocalModelServerStore {
    ...
    public static var servers: [LocalModelServer] {
        get { UserDefaults.standard.data(forKey: storageKey).flatMap { try? JSONDecoder().decode([LocalModelServer].self, from: $0) } ?? [] }
        set {
    ...
    public static func connect(url: String, key: String, replacing existing: LocalModelServer? = nil) async throws -> LocalModelServer {
        ...
        server.url = result.url; server.models = result.models; server.verifiedAt = .now
        // Commit only after verification; a failed replacement keeps the saved connection.
        try DefaultAgentSupport.saveKey(key, provider: server.id)
        var values = servers; values.removeAll { $0.id == server.id }; values.append(server); servers = values
