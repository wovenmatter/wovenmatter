...
extension WorkspaceDatabase {
  private func migrateWorkspaceHistory() throws {
    try transaction {
      try executeUnlocked(
        """
  ...
  private func recordHistoryUnlocked(_ incoming: WorkspaceHistoryEvent) throws {
    var event = incoming
  ...
  private func queryHistoryUnlocked(_ query: WorkspaceHistoryQuery) throws -> GatewayJSONValue {
    var values: [String?] = []
    var sql: String
    ...
    case "conversations":
      sql =
        "SELECT rowid AS sequence,id,title,agent_codename,created_at,updated_at,deleted_at FROM dashboard_conversations WHERE rowid > ? ORDER BY rowid"
    ...
    case "version":
      ...
      sql = "SELECT * FROM note_asset_versions WHERE id=? ORDER BY sequence"
      values = [id]
  ...
  private func historyRowsUnlocked(_ sql: String, values: [String?]) throws -> [GatewayJSONValue] {
    let statement = try prepareUnlocked(sql)
    defer { sqlite3_finalize(statement) }
  ...
  private func checkpointNoteUnlocked(id: String, source: String, force: Bool) throws {
    let note = try prepareUnlocked(
      """
  ...
  public func restoreNoteAssetVersion(noteID: String, versionID: String, expectedRevision: String)
    throws -> NoteEditingResponse
  {
    try transaction {
      let operatorID = try localMutationOperatorIDUnlocked()
...
extension WorkspaceDatabase {
  ...
  private func nextNoteRevisionUnlocked(id: String, now: Date = Date()) throws -> String {
...
extension WorkspaceDatabase {
  private func attachSessionMessageUnlocked(requestID: String, messageID: String) throws {
    let statement = try prepareUnlocked(
      """
