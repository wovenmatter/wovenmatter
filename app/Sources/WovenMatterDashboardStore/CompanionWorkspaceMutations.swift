...
extension WorkspaceDatabase {
  ...
  private func performCompanionMutationUnlocked(_ request: CompanionMutation) throws -> CompanionMutationResult {
    ...
    case .updateNote, .deleteNote:
      ...
      if request.kind == .deleteNote {
        try checkpointNoteUnlocked(id: request.resourceID, source: "before-companion-delete", force: true)
      ...
      try checkpointNoteUnlocked(id: request.resourceID, source: "before-companion-edit", force: true)
  ...
  public func finishCompanionCommand(_ receipt: CompanionCommandReceipt) throws {
    try transaction {
      ...
      try stepDone(update)
      guard changedRowCountUnlocked == 1 else { throw WorkspaceDatabaseError.execute("No reserved command exists.") }
    }
  }
