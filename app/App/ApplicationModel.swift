...
final class ApplicationModel {
    ...
    /// Installs inert launch metadata, never discovering or launching installed CLIs.
    func configureCompanionFixture(directory: URL) async {
        localACPLaunchConfigurations = Dictionary(uniqueKeysWithValues: AgentRuntimeKind.allCases.map {
        ...
        localACPDatabaseReadyRuntimeKinds = Set(AgentRuntimeKind.allCases)
        noteEditingSocketPath = directory.appending(path: "fixture-note.sock").path
        if noteWriteBehind == nil, let dashboardStore {
    ...
    func sendAgentMessage(
        ...
        do {
            _ = try await companionCommands.send(conversation: conversation, input: input, note: note)
            return true
        } catch {
            ensureConversationState(id: conversation.id).setError(error.localizedDescription)
            return false
        }
    }
    ...
    func acceptCanonicalAgentMessage(
        ...
        if requiresIdle && isSteeringActiveTurn { throw LocalACPSessionDatabaseError.runAlreadyActive }
        if expectedRunID != nil && !isSteeringActiveTurn { throw LocalACPSessionDatabaseError.runNotFound }
        if !isSteeringActiveTurn {
            guard localRunningConversationIDs.count
                ...
                throw ApplicationModelError.activeTurnLimitReached
            }
        ...
        do {
            ...
            let usesMediatedNoteEditing = usesOpenClawGateway || conversation.remoteWorkspaceID != nil
            // An already-open desktop note must not prevent a follow-up. A running
            // remote/Gateway turn keeps its existing grant; no new grant is minted.
            let noteBinding = canAgentEditOpenNote(conversation)
                ...
                ? try await makeAgentNoteBinding(
                    ...
                    expectedRevision: expectedNoteRevision
                ) : nil
            ...
            if isSteeringActiveTurn {
                if usesOpenClawGateway {
                    do {
                        let ids = try await dashboardStore.sendActiveOpenClawGatewayPrompt(
                            conversationID: conversation.id,
                            input: normalized,
                            deliveryContent: deliveryContent,
                            expectedRunID: expectedRunID
                        )
                        return LocalACPRunIdentifiers(runID: ids.runID, userMessageID: ids.userMessageID, assistantMessageID: ids.assistantMessageID)
                    } catch LocalACPSessionDatabaseError.steeringUnsupported {
                        throw ApplicationModelError.steeringUnavailable
                    }
                }
                do {
                    let ids = try await dashboardStore.sendActiveLocalACPPrompt(
                        conversationID: conversation.id,
                        input: normalized,
                        deliveryContent: deliveryContent,
                        expectedRunID: expectedRunID
    ...
    func canStartCanonicalLocalSession(_ runtimeKind: AgentRuntimeKind) -> Bool {
        localACPLaunchConfigurations[runtimeKind] != nil
    ...
    func createCanonicalGatewaySession(agentID: UUID, conversationID: String, folderID: String? = nil) async throws -> String {
        guard let dashboardStore,
              openClawGatewayLinks.contains(where: { $0.agentID == agentID && $0.connectionStatus == .ready })
        else { throw CompanionCommandService.CommandError.unknownProvider }
        let id = try await dashboardStore.createLocalACPSession(
            runtimeKind: .openclaw, title: "New OpenClaw chat",
            conversationID: conversationID, gatewayAgentID: agentID, folderID: folderID
        )
    ...
    func createLocalACPSession(
        runtimeKind: AgentRuntimeKind,
        requestedConversationID: String = UUID().uuidString.lowercased(),
        folderID: String? = nil
    ) async -> String? {
        guard canStartCanonicalLocalSession(runtimeKind) else {
    ...
    func cancelCanonicalRun(conversationID: String, runID: String) async throws {
        ...
        if isOpenClawGatewayConversation(conversationID) {
