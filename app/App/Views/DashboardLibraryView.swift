...
struct DashboardLibrarySurface: View {
    ...
    private var query: LibraryQuery {
        var value = selection; value.since = dateRange.start(); return value
    }
    private var library: LibraryModel { model.library }
    private var workspaces: [(String, String)] {
        var values = ["local": "Local workspace"]
        for workspace in model.remoteWorkspaces.workspaces { values[workspace.id.uuidString.lowercased()] = workspace.name }
        for facet in library.facets where values[facet.workspaceID] == nil { values[facet.workspaceID] = facet.workspaceName }
    ...
    private var agents: [(String, String)] {
        ...
        return Dictionary(available.map { ($0.agentID, "\($0.agentName) · \($0.workspaceName)") }, uniquingKeysWith: { first, _ in first })
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if let error = library.error {
                HStack {
                    ...
                    Spacer()
                    Button("Dismiss") { library.error = nil }.buttonStyle(DashboardQuietButtonStyle())
                }.padding(.horizontal, 32).padding(.vertical, 8)
            }
            if library.loading && library.items.isEmpty {
                ProgressView().frame(maxWidth: .infinity, maxHeight: .infinity)
            } else if library.items.isEmpty {
                DashboardConversationEmptyState(icon: .libraryBigControl,
                    title: library.facets.isEmpty ? "No Library items yet" : "No matching items",
                    detail: library.facets.isEmpty ? "Files, links, and photos you exchange in new messages will appear here." : "Adjust your filters or search to find an item.")
                    .frame(maxHeight: .infinity)
            } else {
                ScrollView {
                    LazyVStack(spacing: 8) {
                        ForEach(library.items) { item in
                            DashboardLibraryRow(item: item, onOpen: { library.open(item) },
                                onSource: { onOpenMessage(item) }, onRetry: { library.retry(item) })
                        }
                        if library.hasMore {
                            Button("Load more") { Task { await library.load(query: query, more: true) } }
                                .buttonStyle(DashboardQuietButtonStyle()).disabled(library.loading).padding(.vertical, 12)
                        }
                    }
        ...
        .onChange(of: model.workspaceRevision) { _, _ in Task { await library.load(query: query, preserveCount: true) } }
        .onChange(of: library.revision) { _, _ in Task { await library.load(query: query, preserveCount: true) } }
    }
    private var header: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack(spacing: 10) {
                ...
                } label: { Label(selection.oldestFirst ? "Oldest first" : "Newest first", systemImage: "arrow.up.arrow.down") }
            ...
            ScrollView(.horizontal) {
              HStack(spacing: 16) {
                ...
                Menu {
                    ...
                    ForEach(LibrarySender.allCases, id: \.self) { value in Button(value.title) { selection.sender = value } }
                } label: { filterLabel(selection.sender?.title ?? "All senders") }
                Menu {
                    Button("All types") { selection.kind = nil }
                    ForEach(LibraryItemKind.allCases, id: \.self) { value in Button(value.title) { selection.kind = value } }
                } label: { filterLabel(selection.kind?.title ?? "All types") }
                Menu {
                    ForEach(LibraryDateRange.allCases) { value in Button(value.title) { dateRange = value } }
                } label: { filterLabel(dateRange.title) }
              }
            ...
            .scrollIndicators(.never)
            HStack {
                Text("\(library.items.count)\(library.hasMore ? "+" : "") \(library.items.count == 1 && !library.hasMore ? "item" : "items")").font(.system(size: 12)).foregroundStyle(DashboardPalette.mutedForeground)
                Spacer()
                if selection != LibraryQuery() || dateRange != .all {
                    Button("Clear filters") { selection = .init(); dateRange = .all }.buttonStyle(DashboardQuietButtonStyle())
                }
            }
        }
        .padding(.horizontal, 32).padding(.top, 44).padding(.bottom, 12)
    }
    private func multipleFilter(_ title: String, options: [(String, String)], selected: Binding<Set<String>?>) -> some View {
        Menu {
            Button("All") { selected.wrappedValue = nil }
            Button("None") { selected.wrappedValue = [] }
            Divider()
            ForEach(options, id: \.0) { option in
                Toggle(option.1, isOn: Binding(get: { selected.wrappedValue?.contains(option.0) ?? true }, set: { enabled in
            ...
            filterLabel(selected.wrappedValue.map { "\(title): \($0.isEmpty ? "None" : String($0.count))" } ?? "All \(title.lowercased())")
        }
...
private struct DashboardLibraryRow: View {
    ...
    let onSource: () -> Void
    let onRetry: () -> Void
    private var canOpen: Bool { (item.storage == "link" && item.isWebLink) || item.contentHash != nil }
    var body: some View {
    ...
    private var details: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                Button(action: onOpen) { Text(item.title).font(.system(size: 13, weight: .semibold)).lineLimit(2).multilineTextAlignment(.leading) }
                    .buttonStyle(.plain).disabled(!canOpen).help(item.source)
                ...
                if let date = item.sentDate {
                    Text(date.formatted(date: .abbreviated, time: .shortened)).font(.system(size: 11)).foregroundStyle(DashboardPalette.mutedForeground)
                }
                if let error = item.error {
                    Text(error).font(.system(size: 11)).foregroundStyle(DashboardPalette.mutedForeground).lineLimit(2).help(error)
    ...
    private var actions: some View {
        HStack(spacing: 8) {
            if item.storage == "unavailable" { Button("Retry", action: onRetry).buttonStyle(DashboardQuietButtonStyle()) }
            Button("Open", action: onOpen).buttonStyle(DashboardQuietButtonStyle()).disabled(!canOpen)
...
struct DashboardLibrarySourceSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack { Text(item.conversationTitle).font(.title2); Spacer(); Button("Done") { dismiss() } }
        ...
        .task {
            do { content = try model.dashboardStore?.database.librarySourceMessage(id: item.id) ?? "The workspace is unavailable." }
