...
struct WorkspaceCoordinationEventTests {
  ...
  @Test func receiptPagesAndRefreshKeepEarlierActivityAndExcludeIncomingUpdates() throws {
    ...
    let incoming = try db.reserveToolDelivery(sourceID: b, targetID: a, text: "Incoming", requestID: UUID().uuidString)
    ...
    let refreshed = try db.sessionActivityWindow(sessionID: a, throughID: oldest.id)
    ...
    _ = try db.claimToolDelivery(id: incoming.id)
    try db.markToolDeliveryTransportStarted(id: incoming.id, targetID: a, nativeCommand: "review")
    try db.setToolDeliveryStatus(id: incoming.id, status: "accepted")
    let activityPage = try db.sessionDeliveries(sessionID: a, limit: 2, activityOnly: true)
    #expect(activityPage.map(\.id) == [added.id, incoming.id])
    let previousActivity = try db.sessionDeliveries(sessionID: a, limit: 3, beforeID: incoming.id, activityOnly: true)
    #expect(previousActivity.count == 3 && previousActivity.allSatisfy { $0.sourceID == a })
    let incomingWindow = try db.sessionActivityWindow(sessionID: a, throughID: incoming.id)
