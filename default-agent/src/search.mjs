...
export function searchTools(key, fetchRequest = fetch) {
  async function request(endpoint, body, signal) {
    if (!key) throw new Error('Add an Exa API key in Settings → Default Agent → Search.');
    const response = await fetchRequest(`https://api.exa.ai/${endpoint}`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-api-key': key }, body: JSON.stringify(body), signal: AbortSignal.any([signal ?? new AbortController().signal, AbortSignal.timeout(30000)]) });
