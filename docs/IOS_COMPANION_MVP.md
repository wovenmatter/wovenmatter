...
credentials, SSH routes and concurrency remain on the Mac. This implementation
starts from monorepo main `dc39c49620f1f0777acb2be0d713fe85507d5607`.
...
- Online chat uses the Mac's canonical provider/session service. All eight local
  and remote routes, linked workspaces and configured OpenClaw Gateways are exposed
  with actual availability and negotiated per-session controls. A note-bound send
...
The QR/deep link is `wovenmatter://pair?endpoint=...&token=...&version=1`. Its random
...
routing, atomic folder placement, immutable note bindings and bounded history reads.
...
Test`, so notes and pairing survive reopening. Production workspace paths are
rejected. `WOVENMATTER_COMPANION_TEST_WORKSPACE` is an explicit alternate test path;
