...
Build the current checkout with the repository script. A named variant keeps
its WovenMatter database and process separate from other development builds:
...
2. Enable OpenCode and open its runtime **Settings**, or choose the local
   workspace from **Settings → OpenCode**. **Connect** reuses the standard local
   service or starts it if absent. **Open in browser** opens that service without
   a manual sign-in prompt. Configure provider accounts in OpenCode itself.
...
6. Send a message in a disposable session. Open the same backend session in the browser. Verify both clients' text, tool activity, attachments, permissions, and question replies appear. Include a conditional multi-select question, a suggested string choice with a custom-answer option, and an external step requiring acknowledgement. Avoid sending from both clients simultaneously when checking model selection.
...
These steps include real model calls and belong to manual acceptance with the
